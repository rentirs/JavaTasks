            ...
            if (command.indexOf("getFreeRooms") == 0) { // Допустим мы ввели getFreeRooms wc:true wifi:true (ниже комментарии для этой команды)
                ...
                if(splitted.length == 1) hotel.getFreeRooms(); // Длина массива splitted==1 только тогда, когда мы ввели getFreeRooms и больше ничего
                    ...
                    for (int i = 1; i < splitted.length; i++) { // Перебираем заказанные сервисы, для нашего примера {"wc:true","wifi:true"}
                        ...
                        if(service[0].equals("wc")) wc = service[1].equals("true"); // wc = true
                        else if (service[0].equals("sleepingPlace")) sleepingPlace = Integer.parseInt(service[1]); // sleepingPlace=0
                        else if (service[0].equals("conditioner")) conditioner = service[1].equals("true"); // conditioner = 0
                        else if (service[0].equals("wifi")) wifi = service[1].equals("true");// (на первой иттерации wifi=false) на второй иттерации ей присвоится значение wifi=true
                    }
